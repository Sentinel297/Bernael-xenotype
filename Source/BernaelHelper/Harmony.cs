    ...
    public static class BernaelGraceHarmony
    {
        ...
        public static class Patch_Grace
        {
            ...
            public static void Postfix(ref ResourceGene __instance)
            {
                ...
                if (!__instance.pawn.IsHashIntervalTick(180)|| __instance.def != BernaelDefOf.GS_Grace_New ||
                    __instance.Resource.Value > 0 || !pawn.InMentalState) return;
        ...
        public static class Patch_GraceIngestion
        {
            ...
            public static void Postfix(Pawn pawn, Thing ingested)
            {
                ...
                foreach (Pawn otherPawn in pawn.Map.mapPawns.AllPawnsSpawned)
                {
                    ...
                    var memory = otherPawn.needs.mood.thoughts;
        ...
        {
            public static FieldInfo pawnField => AccessTools.Field(typeof(Pawn_InteractionsTracker), "pawn");
            public static void Postfix(Pawn_InteractionsTracker __instance, ref bool __result, Pawn recipient)
            {
                if (!__result ) return;
                Pawn instigator = (Pawn)pawnField.GetValue(__instance);
                if (instigator.genes == null || instigator.genes.GetGene(BernaelDefOf.BX_DarkSpeech) == null) return;
